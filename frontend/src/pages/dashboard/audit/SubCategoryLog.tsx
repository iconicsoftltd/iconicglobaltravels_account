...
const SubCategoryLogPage: React.FC = () => {
  ...
  const {
    data: singleLogData,
  } = useGetSubCategoryLogByIdQuery(selectedLogId, { skip: !selectedLogId });

  const logs: SubCategoryLogType[] = logData?.data || [];
  ...
  const columns: ColumnDef<SubCategoryLogType>[] = [
    {
      id: "sl",
      header: "SL",
  ...
  return (
    <div className="p-4 min-h-screen space-y-4">
      ...
      <ReusableTable<SubCategoryLogType>
        columns={columns}
        data={logs}
        currentPage={page}
...
export default SubCategoryLogPage;
