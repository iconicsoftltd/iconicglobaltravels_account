...
const ServiceSalesLogPage: React.FC = () => {
  ...
  const {
    data: singleLogData,
  } = useGetServiceSalesLogByIdQuery(selectedLogId, { skip: !selectedLogId });

  const logs: ServiceSalesLogType[] = logData?.data || [];
  ...
  const columns: ColumnDef<ServiceSalesLogType>[] = [
    {
      id: "sl",
      header: "SL",
  ...
  return (
    <div className="p-4 min-h-screen space-y-4">
      ...
      <ReusableTable<ServiceSalesLogType>
        columns={columns}
        data={logs}
        currentPage={page}
...
export default ServiceSalesLogPage;
