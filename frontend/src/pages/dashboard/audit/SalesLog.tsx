...
const SalesLogPage: React.FC = () => {
  ...
  const {
    data: singleLogData,
  } = useGetSalesLogByIdQuery(selectedLogId, { skip: !selectedLogId });

  const logs: SalesLogType[] = logData?.data || [];
  ...
  const columns: ColumnDef<SalesLogType>[] = [
    {
      id: "sl",
      header: "SL",
