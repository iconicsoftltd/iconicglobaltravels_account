...
const QuotationLogPage: React.FC = () => {
  ...
  const {
    data: singleLogData,
  } = useGetQuotationLogByIdQuery(selectedLogId, { skip: !selectedLogId });

  const logs: QuotationLogType[] = logData?.data || [];
  ...
  const columns: ColumnDef<QuotationLogType>[] = [
    {
      id: "sl",
      header: "SL",
  ...
  return (
    <div className="p-4 min-h-screen space-y-4">
      ...
      <ReusableTable<QuotationLogType>
        columns={columns}
        data={logs}
        currentPage={page}
...
export default QuotationLogPage;
