...
interface SalaryStructureLogType {
  id: number;
  branchId: number;
  chequeId: number;
  ip: string;
...
const SalaryStructureLogPage: React.FC = () => {
  ...
  const {
    data: singleLogData,
  } = useGetSalaryStructureLogByIdQuery(selectedLogId, { skip: !selectedLogId });

  const logs: SalaryStructureLogType[] = logData?.data || [];
  ...
  const columns: ColumnDef<SalaryStructureLogType>[] = [
    {
      id: "sl",
      header: "SL",
  ...
  return (
    <div className="p-4 min-h-screen space-y-4">
      ...
      <ReusableTable<SalaryStructureLogType>
        columns={columns}
        data={logs}
        currentPage={page}
...
export default SalaryStructureLogPage;
