...
interface LeaveDaySetupLogType {
  id: number;
  branchId: number;
  chequeId: number;
  ip: string;
...
const LeaveDaySetupLogPage: React.FC = () => {
  ...
  const {
    data: singleLogData,
  } = useGetLeaveDaySetupLogByIdQuery(selectedLogId, { skip: !selectedLogId });

  const logs: LeaveDaySetupLogType[] = logData?.data || [];
  ...
  const columns: ColumnDef<LeaveDaySetupLogType>[] = [
    {
      id: "sl",
      header: "SL",
  ...
  return (
    <div className="p-4 min-h-screen space-y-4">
      ...
      <ReusableTable<LeaveDaySetupLogType>
        columns={columns}
        data={logs}
        currentPage={page}
...
export default LeaveDaySetupLogPage;
