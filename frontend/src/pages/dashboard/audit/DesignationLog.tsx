...
const DesignationLogPage: React.FC = () => {
  ...
  const {
    data: singleLogData,
  } = useGetDesignationLogByIdQuery(selectedLogId, { skip: !selectedLogId });

  const logs: DesignationLogType[] = logData?.data || [];
  ...
  const columns: ColumnDef<DesignationLogType>[] = [
    {
      id: "sl",
      header: "SL",
  ...
  if (error)
    return (
      <div className="text-center text-red-500 p-8">
        Error loading Designation logs
      </div>
    );
  ...
  return (
    <div className="p-4 min-h-screen space-y-4">
      ...
      <ReusableTable<DesignationLogType>
        columns={columns}
        data={logs}
        currentPage={page}
...
export default DesignationLogPage;
