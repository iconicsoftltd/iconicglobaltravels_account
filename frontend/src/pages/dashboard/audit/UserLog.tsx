...
const UserLogPage: React.FC = () => {
  ...
  const {
    data: singleLogData,
  } = useGetUserLogByIdQuery(selectedLogId, { skip: !selectedLogId });

  const logs: UserLogType[] = logData?.data || [];
  ...
  const columns: ColumnDef<UserLogType>[] = [
    {
      id: "sl",
      header: "SL",
