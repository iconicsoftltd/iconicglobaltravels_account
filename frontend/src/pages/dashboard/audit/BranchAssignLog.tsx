...
const BranchAssignLogPage: React.FC = () => {
  ...
  const {
    data: singleLogData,
  } = useGetBranchAssignLogByIdQuery(selectedLogId, { skip: !selectedLogId });

  const logs: BranchAssignLogType[] = logData?.data || [];
  ...
  const columns: ColumnDef<BranchAssignLogType>[] = [
    {
      id: "sl",
      header: "SL",
  ...
  return (
    <div className="p-4 min-h-screen space-y-4">
      ...
      <ReusableTable<BranchAssignLogType>
        columns={columns}
        data={logs}
        currentPage={page}
...
export default BranchAssignLogPage;
