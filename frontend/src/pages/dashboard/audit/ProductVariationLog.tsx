...
interface ProductVariationLogType {
  id: number;
  branchId: number;
  chequeId: number;
  ip: string;
...
const ProductVariationLogPage: React.FC = () => {
  ...
  const {
    data: singleLogData,
  } = useGetProductVariationLogByIdQuery(selectedLogId, { skip: !selectedLogId });

  const logs: ProductVariationLogType[] = logData?.data || [];
  ...
  const columns: ColumnDef<ProductVariationLogType>[] = [
    {
      id: "sl",
      header: "SL",
  ...
  return (
    <div className="p-4 min-h-screen space-y-4">
      ...
      <ReusableTable<ProductVariationLogType>
        columns={columns}
        data={logs}
        currentPage={page}
...
export default ProductVariationLogPage;
