...
const LeaveApplyLogPage: React.FC = () => {
  ...
  const {
    data: singleLogData,
  } = useGetLeaveApplyLogByIdQuery(selectedLogId, { skip: !selectedLogId });

  const logs: LeaveApplyLogType[] = logData?.data || [];
  ...
  const columns: ColumnDef<LeaveApplyLogType>[] = [
    {
      id: "sl",
      header: "SL",
  ...
  return (
    <div className="p-4 min-h-screen space-y-4">
      ...
      <ReusableTable<LeaveApplyLogType>
        columns={columns}
        data={logs}
        currentPage={page}
...
export default LeaveApplyLogPage;
