...
interface PurchaseReturnLogType {
  id: number;
  branchId: number;
  chequeId: number;
  ip: string;
...
const PurchaseReturnLogPage: React.FC = () => {
  ...
  const {
    data: singleLogData,
  } = useGetPurchaseReturnLogByIdQuery(selectedLogId, { skip: !selectedLogId });

  const logs: PurchaseReturnLogType[] = logData?.data || [];
  ...
  const columns: ColumnDef<PurchaseReturnLogType>[] = [
    {
      id: "sl",
      header: "SL",
  ...
  return (
    <div className="p-4 min-h-screen space-y-4">
      ...
      <ReusableTable<PurchaseReturnLogType>
        columns={columns}
        data={logs}
        currentPage={page}
...
export default PurchaseReturnLogPage;
