...
const DepartmentLogPage: React.FC = () => {
  ...
  const {
    data: singleLogData,
  } = useGetDepartmentLogByIdQuery(selectedLogId, { skip: !selectedLogId });

  const logs: DepartmentLogType[] = logData?.data || [];
  ...
  const columns: ColumnDef<DepartmentLogType>[] = [
    {
      id: "sl",
      header: "SL",
  ...
  return (
    <div className="p-4 min-h-screen space-y-4">
      ...
      <ReusableTable<DepartmentLogType>
        columns={columns}
        data={logs}
        currentPage={page}
